...
export const generateContractPDF = (data, returnBlob = false) => {
  const doc = new jsPDF();
  ...
  if (data.hourlyRate) {
    doc.text(
      `Tarifa por hora: $${parseFloat(data.hourlyRate).toFixed(2)}`,
      14,
      currentY
    );
    currentY += 8;
  ...
  if (data.totalAmount) {
    doc.text(
      `Monto total acordado: $${parseFloat(data.totalAmount).toFixed(2)}`,
      14,
      currentY
    );
    currentY += 10;
...
`;
  ...
  const addSignature = (sigData, x, y, label, name) => {
    try {
      doc.addImage(
        sigData || DEFAULT_SIGNATURE_BASE64,
        "PNG",
        x,
        y,
        imgWidth,
      ...
      console.warn(
        `Firma inválida para ${label}, usando predeterminada:`,
        e.message
      );
