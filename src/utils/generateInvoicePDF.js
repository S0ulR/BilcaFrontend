...
export const generateInvoicePDF = (data, returnBlob = false) => {
  const doc = new jsPDF();
  ...
  doc.text(
    `Fecha de emisión: ${data.date || new Date().toLocaleDateString()}`,
    14,
    40
  );
  ...
  autoTable(doc, {
    ...
    body: [
      [data.worker.name, data.worker.profession || "Trabajador independiente"],
    ],
    theme: "grid",
    styles: { fontSize: 10 },
  });
  ...
  if (data.startDate || data.endDate) {
    ...
    if (data.endDate)
      doc.text(
        `Fecha tentativa de finalización: ${data.endDate}`,
        100,
        currentY
      );
    currentY += 10;
  ...
  doc.setFontSize(10);
  const warningLines = warnings.flatMap((line) =>
  ...
  doc.text(
    "Documento generado digitalmente – No requiere firma manuscrita.",
    14,
    finalY4 + 8
  );

  const fileName = `factura_${data.invoiceNumber}_${
    data.client.name
