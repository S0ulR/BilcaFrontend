...
const WorkerServicesForm = ({ onUpdateUser }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await API.put('/api/users/services', { services });
  ...
  return (
    <div className="worker-services-form">
      <h2>{user.role === "worker" ? "Gestionar mis servicios" : "Ofrecer mis servicios"}</h2>
      <p>
        {user.role === "worker"
          ...
          {services.map((service, index) => (
            <div key={index} className="service-row">
              <select
                ...
                onChange={(e) => updateService(index, 'profession', e.target.value)}
                required
              >
              ...
              <input
                ...
                onChange={(e) => updateService(index, 'hourlyRate', e.target.value)}
                required
              />
              <textarea
